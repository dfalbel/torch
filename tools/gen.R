...
argument_string <- function(argument) {
  ...
  glue::glue("{if (nullable) 'Rcpp::Nullable<' else ''}{argument$dynamic_type}{if (nullable) '>' else ''} {argument$name}")
}

arguments_string <- function(arguments) {
  arguments <- purrr::map_chr(arguments, argument_string)
...
arguments_preprocess <- function(arguments) {
  paste(purrr::map_chr(arguments, ~.x$name), collapse = ", ")
}
